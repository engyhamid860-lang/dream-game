...
function initApp() {
  ...
  function drawWheel(angle = 0, liveTimerVal = null) {
    ...
    // Medallion Outer Bevel Ring (تكبير دائرة العد التنازلي المركزية بوضوح ضخم وعملاق)
    wheelCtx.save();
    wheelCtx.beginPath();
    wheelCtx.arc(centerX, centerY, 65, 0, Math.PI * 2);
    ...
    const coreGrad = wheelCtx.createRadialGradient(centerX, centerY, 5, centerX, centerY, 52);
    if (isWarning) {
    ...
    if (displayTimer !== null && displayTimer !== undefined && displayTimer > 0) {
      ...
      wheelCtx.font = '900 40px Outfit, Cairo, sans-serif';
      wheelCtx.fillStyle = isWarning ? '#ffffff' : '#ffd700';
      wheelCtx.shadowColor = isWarning ? '#ef4444' : '#ffd700';
      wheelCtx.shadowBlur = 12;
      wheelCtx.fillText(formatted, centerX, centerY + 2);
    } else {
      wheelCtx.font = '900 22px Cairo, sans-serif';
