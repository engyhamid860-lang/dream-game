...
function initApp() {
  ...
  function drawWheel(angle = 0, liveTimerVal = null) {
    ...
    // Medallion Outer Bevel Ring
    wheelCtx.save();
    wheelCtx.beginPath();
    wheelCtx.arc(centerX, centerY, 34, 0, Math.PI * 2);
    ...
    wheelCtx.beginPath();
    wheelCtx.arc(centerX, centerY, 27, 0, Math.PI * 2);
    const coreGrad = wheelCtx.createRadialGradient(centerX, centerY, 5, centerX, centerY, 27);
    if (isWarning) {
    ...
    if (displayTimer !== null && displayTimer !== undefined && displayTimer > 0) {
      ...
      wheelCtx.font = '900 21px Outfit, Cairo, sans-serif';
      ...
      wheelCtx.font = '900 13px Cairo, sans-serif';
